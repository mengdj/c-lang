...
WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
	if (waveInGetNumDevs()) {
		...
		HANDLE hTmp = CreateFile("rec.pcm", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_TEMPORARY | FILE_ATTRIBUTE_HIDDEN, NULL);
		if (hTmp != NULL) {
			for (int i = 0; i < FRAGMENT_NUM; i++) {
				wh[i].lpData = pBuffer + (i*FRAGMENT_SIZE);
				wh[i].dwBufferLength = FRAGMENT_SIZE;
				wh[i].dwBytesRecorded = 0;
				wh[i].dwUser = (DWORD_PTR)hTmp;
				wh[i].dwFlags = 0;
				wh[i].dwLoops = 1;
				wh[i].lpNext = NULL;
				wh[i].reserved = 0;
...
void CALLBACK waveInProc(HWAVEIN hwi, UINT uMsg, DWORD dwInstance, DWORD dwParam1, DWORD dwParam2) {
	if (WIM_DATA == uMsg) {
		...
		HANDLE hTmp = (HANDLE)pwh->dwUser;
		if (hTmp != NULL) {
			DWORD dwreturnsize;
			WriteFile(hTmp, pwh->lpData, pwh->dwBytesRecorded, &dwreturnsize, NULL);
		}
		EnterCriticalSection(&cs);
		if (stopWrite == FALSE) {
